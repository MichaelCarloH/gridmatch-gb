  ...
  const renderTab = () => {
    if (tab === 'overview') return <div className="grid two">
      ...
      <article className="card dark"><h3>Operational brief</h3><div className="rank-list">
        ...
        <div className="rank-row"><i>M</i><span>Model MAE</span><b>{metrics.loading ? <span className="inline-skeleton" aria-label="Loading" /> : metrics.error ? 'Unavailable' : `${number(mlMetric?.mae, 3)} MWh`}</b></div>
        <div className="rank-row"><i>A</i><span>Active alerts</span><b>{alerts.loading ? <span className="inline-skeleton" aria-label="Loading" /> : alerts.error ? 'Unavailable' : alerts.data?.meta?.count}</b></div>
      </div></article>
    </div>;
    ...
    if (tab === 'weather') {
      if (forecast.loading || forecast.error) return <DataState loading={forecast.loading} error={forecast.error} />;
    ...
    if (tab === 'model') {
      if (metrics.loading || modelCard.loading || metrics.error || modelCard.error) return <DataState loading={metrics.loading || modelCard.loading} error={metrics.error || modelCard.error} />;
    ...
    if (tab === 'quality') {
      if (quality.loading || quality.error) return <DataState loading={quality.loading} error={quality.error} />;
      return <article className="card"><h3>Readiness and quality evidence</h3><div className="detail-list">{Object.entries(quality.data?.data ?? {}).filter(([, value]) => ['string', 'number', 'boolean'].includes(typeof value)).slice(0, 14).map(([key, value]) => <div key={key}><span>{titleCase(key)}</span><b>{String(value)}</b></div>)}</div></article>;
    }
    if (observations.loading || metrics.loading || observations.error || metrics.error) return <DataState loading={observations.loading || metrics.loading} error={observations.error || metrics.error} />;
